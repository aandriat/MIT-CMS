      ...
      for(Int_t ientry=0; ientry<max_events; ientry++) {
        ...
        Bool_t mass_Zcut=kTRUE;
          ...
          if (snamev[isam]=="zmm"){
            ...
            toolbox::fillGen(genPartArr, BOSON_ID, gvec, glep1, glep2, &glepq1, &glepq2,0); // Selects the boson and daughter leptons, stores 4-vectors and charge
            if (gvec->M() < 60 || gvec->M() > 120){ // definition of z-boson
              mass_Zcut=kFALSE;
            }
            if ((glep1->Pt() > 25) && (glep2->Pt() > 25) && (TMath::Abs(glep1->Eta()) < 2.4) && (TMath::Abs(glep2->Eta()) < 2.4) && (gvec->M() > 60 && gvec->M() < 120)){ // acceptance definition
              fiducial=1; // if event meets acceptance criteria for being in the fiducial region, stores fiducial as 1. 
            }
